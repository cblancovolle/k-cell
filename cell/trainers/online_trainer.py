...
from cell.common.utils import clipmin_first_if_all
...
class OnlineTrainer:
    ...
    def learn_one(self, x_new: Tensor, y_new: Tensor):
        ...
        if n_neighbors <= 1:
            ...
            try:
                ...
                y_baseline = self.short_term_buffer_data[1].mean(dim=0)
        ...
        if (
            ...
            try:
                ...
                y_hat_new = self.buffer_data[1].mean(dim=0)
    ...
    def predict_one(self, x_test: Tensor):
        ...
        # print("Closest k and weights (Trainer)", closest, weights)
